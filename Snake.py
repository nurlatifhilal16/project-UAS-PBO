...
##            self.allWallslist.add(i[0])
    ...
    def Wall2(self,screen) :
        ...
        wall = [[self.wall1,self.rect1],[self.wall2,self.rect2],
                [self.wall3,self.rect3],[self.wall4,self.rect4],
                [self.wall5,self.rect5]]
        for i in wall:
            screen.blit(i[0],i[1])
...
class APP:
    ...
    def game_init(self) :
        self.game_bound = {
            ...
            'min_y' : 100,
            'max_y' : 600 }
        self.food = Food((self.game_bound['min_x'] + WALL_THICKNESS,
                          self.game_bound['max_x']-WALL_THICKNESS),
                        (self.game_bound['min_y'] + WALL_THICKNESS,
                         self.game_bound['max_y']-WALL_THICKNESS))
        self.food.spawn()
        self.score = 0
        self.level = 1
        self.wall = Wall()
